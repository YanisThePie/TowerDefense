...
public class VolumetricComponent : ExploderComponent {
	...
	private void initParticleSystem() {
        gameObject.GetComponent<ParticleSystem>().maxParticles = maxParticles;
        gameObject.GetComponent<ParticleSystem>().emissionRate = 0;
        gameObject.GetComponent<ParticleSystem>().startSpeed = 0;
        gameObject.GetComponent<ParticleSystem>().startSize = 1.0f;
        gameObject.GetComponent<ParticleSystem>().simulationSpace = ParticleSystemSimulationSpace.World;
        gameObject.GetComponent<ParticleSystem>().startLifetime = duration;

        gameObject.GetComponent<ParticleSystem>().Emit(startEmission);
		curCount = gameObject.GetComponent<ParticleSystem>().GetParticles(particles);
        ...
        gameObject.GetComponent<ParticleSystem>().SetParticles(particles, curCount);
	}

	protected void emitNewParticles() {
		if ((Time.time - exploder.explosionTime) / duration < centerEmissionDuration) {
            gameObject.GetComponent<ParticleSystem>().Emit(Mathf.Min((int) (centerEmission * Time.deltaTime), maxParticles - curCount));
		} else {
            gameObject.GetComponent<ParticleSystem>().Emit(Mathf.Min((int) (emission * Time.deltaTime), maxParticles - curCount));
		}
		int nextCount = gameObject.GetComponent<ParticleSystem>().GetParticles(particles);
